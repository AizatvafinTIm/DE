...
class ImprovedEuler(Method):
    ...
    def get_y_by_method(self, x_prev, y_prev):
        return y_prev + self.h * self.y_prime(x_prev + (self.h / 2),
                                              y_prev + (self.h / 2) * self.y_prime(x_prev, y_prev))

    def count_ieuler(self):
        self.euler_arr = [0] * len(self.interval)
        self.euler_arr[0] = self.y0
        for i in range(1, len(self.interval)):
            self.euler_arr[i] = float(
                self.get_y_by_method(self.interval[i - 1], self.euler_arr[i - 1]))
    ...
    def count_gte(self):
        ...
        self.euler_arr = self.count_ieuler()
